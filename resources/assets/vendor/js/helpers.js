...
const Helpers = {
  ...
  initials: function initials(str) {
    return str.split(' ').map(x => x.charAt(0)).join('').substr(0, 2).toUpperCase();
  },
  // ---
  ...
  handleAlertVisibility: function handleAlertVisibility(element, status, className = null, message = null) {
    let globalAlert = element;
  ...
  customInputErrorsValidation: function customInputErrorsValidation(errors, wrapperEl = null) {
  ...
  // ---
  // Format amount
  formatAmount: function formatAmount(value, decimal = 0, prefix = '', suffix = '') {
    value += '';
    let x = value.split('.');
    let x1 = x[0];
  ...
  renderToastProgress: function toast(domElement, message) {
    ...
    domElement.find('div.toast-body').empty().append($('' +
      '<div class="d-flex align-items-start mb-4">' +
      '<div class="spinner-border spinner-border-md text-secondary" role="status"></div>' +
      '<span class="ps-4 fs-6 text-truncate" style="padding-top: 0.40rem;">'+ message +' ...</span>' +
      '</div>'
    ));
  },
  // ---
  ...
  renderToastInfo: function toast(domElement, message) {
    ...
    domElement.find('div.toast-body').empty().append($('' +
      ...
      '<span class="ps-4 fs-6 text-wrap" style="padding-top: 0.16rem;">'+ message +'</span></div>'
    ));
  },
  // ---
  // Init editor
  ...
  getMultipleSelectValues: function getMultipleSelectValues(select) {
    let result = [];
    let options = select && select.find('option');
    let opt;
  ...
  generateReferenceNumber: function generateReferenceNumber(value, count = 7) {
    let strValue = value.toString();
    if(strValue.length >= count) return value;

    return `${'0'.repeat(count - strValue.length)}${value}`;
  },
}
...
_Toast.prototype.init = function(domElement, options = {}){
  this.domWrapper = $(domElement);
  this.toastAnimation = new bootstrap.Toast(domElement, {
    ...options,
    autohide: false
  });
}
...
_Toast.prototype.show = function(dispose = false, header = null, body = null){
  this.domWrapper.find('button.btn-close').removeClass('d-none');
