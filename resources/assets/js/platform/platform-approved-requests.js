...
$(function () {
  let approvedRequestsTable = $('#approvedRequestsTable'), dtBasic;
  ...
  function loadInitialDataPage() {
    $.ajax({
      url: window.variables.routes.approved_request,
      type: "POST",
      dataType : 'json',
      data: buildFilters(),
      beforeSend: function () {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<span class="w-px-250 text-wrap" style="">'+ full.display_type +'</span>' +
              '</div>'
            );
          }
        },
        ...
        },
        {
          ...
          className: 'fw-semibold',
          render: function (data, type, full, meta) {
            if(full.operating_title !== null) {
              return full.operating_title.display_owner;
            }

            if(full.approval !== null) {
              return full.approval.display_owner;
            }
        ...
        },
        {
          targets: 6,
          render: function (data, type, full, meta) {
            let utcDate = moment.utc(full.approved_at);
            return moment(utcDate).local().format(window.variables.messages.js_date_format);
          }
        },
        {
          ...
          searching: false,
          render: function (data, type, full, meta) {
            let wrapper = $('<div class=""></div>').wrap('<div />');
            let docs = full.operating_title !== null ? full.operating_title.attachments : (full.approval !== null ? full.approval.attachments : [] );

            if(docs.length > 0) {
              docs.forEach(function (doc) {
                if(doc.attachment_url !== null) {
                  wrapper.append('<p class="m-0 p-0 pb-0 text-wrap w-px-300">' +
                    '<a href="'+ doc.attachment_url +'" class="text-primary fw-semibold text-decoration-none" target="_blank" style="font-size: 0.94rem;">' +
                    '' + doc.attachment_type.designation + '' +
                    '</a>' +
                    '<span class="badge bg-label-secondary d-inline-block px-1 text-uppercase ms-2">' +
                    '' + doc.path.split('.').reverse()[0].toUpperCase() + '' +
                    '</span>' +
                    '</p>'
                  );
                }
            ...
            return '<p class="m-0 p-0 text-muted">'+ window.variables.messages.empty_docs +'</p>';
          }
        },
        {
          targets: 8,
        ...
        },
        {
          ...
          searching: false,
          render: function (data, type, full, meta) {
            let url = window.variables.routes.approved_request_details.replace(/id_/gi, full.id);
  ...
  function initDataTable() {
    if (approvedRequestsTable.length > 0) {
      dtBasic = approvedRequestsTable.DataTable(
