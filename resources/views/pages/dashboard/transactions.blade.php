...
new class extends Component {
    ...
    public function closeModal()
    {
        ...
        $this->reset(['client', 'montant', 'statut', 'date']);
    }
        ...
        <div class="container mx-auto px-2 mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white rounded-2xl shadow-lg p-6 text-center transition transform hover:-translate-y-2 hover:shadow-2xl">
                    ...
                    <p class="text-3xl font-bold text-indigo-600">700</p>
                </div>
                <div class="bg-white rounded-2xl shadow-lg p-6 text-center transition transform hover:-translate-y-2 hover:shadow-2xl">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">Montant Total</h3>
                    <p class="text-3xl font-bold text-indigo-600">15000 FCFA</p>
                </div>
                <div class="bg-white rounded-2xl shadow-lg p-6 text-center transition transform hover:-translate-y-2 hover:shadow-2xl">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">Transactions Réussies</h3>
                    <p class="text-3xl font-bold text-indigo-600">650</p>
                </div>
            </div>

            <!-- Add Button -->
            <div class="flex justify-end mb-4">
                <button wire:click="openModal" class="bg-indigo-600 text-white py-2 px-6 rounded-lg hover:bg-indigo-700 transition duration-300">Ajouter Transaction</button>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody>
                        @foreach ([['id' => 1, 'client' => 'Jean Dupont', 'montant' => '99.99 FCFA', 'statut' => 'Réussie', 'date' => '2025-07-20'], ['id' => 2, 'client' => 'Marie Curie', 'montant' => '49.99 FCFA', 'statut' => 'En attente', 'date' => '2025-07-19']] as $transaction)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="py-3 px-4">{{ $transaction['id'] }}</td>
                                <td class="py-3 px-4">{{ $transaction['client'] }}</td>
                                <td class="py-3 px-4">{{ $transaction['montant'] }}</td>
                                <td class="py-3 px-4">{{ $transaction['statut'] }}</td>
            ...
            <div wire:model="showModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center {{ $showModal ? '' : 'hidden' }}">
                <div class="bg-white rounded-lg p-6 w-full max-w-md">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">Ajouter une Transaction</h2>
                    <form wire:submit.prevent="addTransaction">
                        <div class="mb-4">
                            <label class="block text-gray-700 mb-2" for="client">Client</label>
                            <select id="client" wire:model="client" class="w-full p-2 border rounded-lg" required>
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 mb-2" for="montant">Montant (€)</label>
                            <input type="number" id="montant" wire:model="montant" step="0.01" class="w-full p-2 border rounded-lg" required>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" id="date" wire:model="date" class="w-full p-2 border rounded-lg" required>
                        </div>
