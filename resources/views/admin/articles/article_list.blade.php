...
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">eCommerce /</span> Liste des articles
</h4>
    ...
    <img style="" src="{{ asset($item->{'image'}) }}" alt="">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
               ...
               <a href="{{route('status.change',['id'=>$item->id, 'base'=>'articles' , 'status'=>'Success' ])}}"    class='badge bg-success' >Activer</a>
               @endif
               @if($item->status == "Success")
               <a href="{{route('status.change',['id'=>$item->id, 'base'=>'articles' , 'status'=>'Pending' ])}}"    class='badge bg-danger' >Desactiver</a>
               @endif
