...
(function () {
  ...
  if (language !== null && langDropdown.length) {
    ...
    //let setLangFlag = (document.querySelector('.dropdown-language .dropdown-toggle').childNodes[1].className = selectedFlag);
  }
...
class Common {
  ...
  static async askConfirmation({ title = '', message, theme = ConfirmationTheme.white, confirmLabel, cancelLabel }) {
    let confirmed = false;
  ...
  static formatAmount(amount, decimals = 0, decimalSeparator = '', thousandsSeparator = ' ') {
    amount = amount * 1;
    let str = amount
    ...
    str[0] = parts.join(thousandsSeparator ? thousandsSeparator : ',');
    return str.join(decimalSeparator ? decimalSeparator : '.');
  }
}
...
class Loader {
  static setLoading($element) {
    ...
    $element.html(`<i class='fa fa-spinner fa-spin '></i> ${originalHtml}`);
    $element.attr('disabled', '');
    $element.attr('original', originalHtml);
  }

  static removeLoading($element) {
    let originalHtml = $element.attr('original');
