...
/**
 * Les menus du tableau de bord, et qui a le droit de les voir.
 *
 * La liste vivait dans le gabarit de la barre de navigation. L'écran des droits
 * d'accès doit lire exactement la même : recopiée, elle aurait divergé, et on
 * aurait accordé des droits sur des menus disparus tout en en oubliant de
 * nouveaux.
 *
 * La clé d'un menu est le nom de sa route. Ce n'est pas un hasard : c'est ce qui
 * permet de vérifier un droit à partir de la page demandée, sans table de
 * correspondance à tenir à jour.
 */
class MenuTableauDeBord
{
    /**
     * Sections de la navigation : [titre, [[route, libellé, icône], …]].
     *
    ...
    public static function sections(): array
    {
            ...
            ['Catalogue', [
                ...
                ['dashboard.promotions', 'Promotions', 'M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z M6 6h.008v.008H6V6z'],
            ...
            ['Activité', [
                ...
                ['dashboard.retraits', 'Retraits agents', 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
            ...
            */
            ['Boutiques', [
                ...
                // commission majorant les prix affichés, ou abonnement.
                ['dashboard.finance', 'Finance', 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
            ]],
            ['Personnes', [
            ...
            ['Réglages', [
                ...
                ['dashboard.droits', "Droits d'accès", 'M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z'],
            ]],
            ];
    }

    /**
     * Toutes les routes de menu, à plat.
     *
    ...
    public static function routes(): array
    {
        return collect(self::sections())
            ->flatMap(fn (array $section) => collect($section[1])->pluck(0))
            ->values()
            ->all();
    }

    /**
     * Libellé d'un menu, ou sa route si elle n'est plus au menu.
     */
    public static function libelle(string $route): string
    {
        foreach (self::sections() as [$titre, $liens]) {
    ...
    /**
     * L'accueil, toujours accessible.
     *
     * Un employé sans aucun droit doit tout de même pouvoir se connecter et
     * atterrir quelque part, faute de quoi il tourne en boucle sur un refus.
     */
    public const TOUJOURS_OUVERT = ['admin.index'];

    /**
     * Menus visibles par cet utilisateur.
     *
     * Un administrateur voit tout : lui retirer des menus reviendrait à créer un
     * tableau de bord que personne ne peut administrer entièrement.
     *
     * @return array<int, array{0: string, 1: array<int, array{0: string, 1: string, 2: string}>}>
     */
    public static function pourUtilisateur(?\App\Models\User $utilisateur): array
    {
        if (! $utilisateur || $utilisateur->role === 'admin') {
            return self::sections();
        }

        $accordes = self::droitsDe($utilisateur);

        return collect(self::sections())
            ->map(fn (array $section) => [
                $section[0],
                collect($section[1])
                    ->filter(fn (array $lien) => in_array($lien[0], $accordes, true))
                    ->values()
                    ->all(),
            ])
            // Une section vidée de tous ses liens ne doit pas laisser un titre
            // orphelin dans la barre.
            ->filter(fn (array $section) => count($section[1]) > 0)
            ->values()
            ->all();
    }

    /**
     * Routes autorisées pour cet utilisateur, l'accueil compris.
     *
     * @return array<int, string>
     */
    public static function droitsDe(\App\Models\User $utilisateur): array
    {
        if ($utilisateur->role === 'admin') {
            return self::routes();
        }

        return \App\Models\MenuPermission::where('user_id', $utilisateur->id)
            ->pluck('menu')
            ->merge(self::TOUJOURS_OUVERT)
            ->unique()
            ->values()
            ->all();
    }
     ...
     */
    public static function autorise(?\App\Models\User $utilisateur, ?string $route): bool
    {
        if (! $utilisateur) {
            return false;
        }
        ...
        /*
         | Une route hors menu reste ouverte.
         |
         | Le tableau de bord compte des pages qui ne figurent pas dans la barre
         | — actions POST, flux de rafraîchissement, sous-pages. Les refuser
         | faute de droit explicite casserait des écrans auxquels l'employé a
         | pourtant accès.
         */
        ...
        return in_array($route, self::droitsDe($utilisateur), true);
    }
}
